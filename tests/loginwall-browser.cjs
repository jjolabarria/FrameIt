...
(async () => {
 ...
 await page.locator('.user-menu-trigger').click();await page.getByRole('menuitem',{name:'Cerrar sesión',exact:true}).click();await page.waitForURL('**/acceso?**');await second.waitForURL('**/acceso?**');assert.equal(await second.locator('.workspace-shell').count(),0);results.push('Logout removes backoffice in both tabs');
