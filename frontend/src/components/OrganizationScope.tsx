...
export function OrganizationScope() {
  ...
  if (!auth.isAdmin) return auth.organizationName ? <p className="organization-label">{auth.organizationName}</p> : null
  let selected = ''
  try { selected = sessionStorage.getItem('frameit.admin-organization') ?? '' } catch { /* Optional storage. */ }
  return <div className="organization-scope"><label>Organización<select aria-label="Organización de trabajo" value={selected} disabled={!data} onChange={event => {
