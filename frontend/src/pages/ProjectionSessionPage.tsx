...
import { api, getTimeRemaining } from '../lib/api'
import type { SessionJourney } from '../types'
...
import { ProjectedConsolidation } from '../components/ResponseConsolidation'
import { VotingResults } from '../components/VoteFromResults'

const SessionJourneyView = lazy(() => import('../components/SessionJourney').then(module => ({ default: module.SessionJourneyView })))
...
export function ProjectionSessionPage() {
  ...
  useEffect(() => {
    if (!snapshot?.journeyPlayback || snapshot.journeyPlayback.state === 'Stopped') { setJourney(null); return }
    const controller = new AbortController()
    void api<SessionJourney>(`/api/sessions/by-code/${code}/journey`, { signal: controller.signal }).then(setJourney).catch(() => setJourney(null))
    return () => controller.abort()
  }, [code, snapshot?.journeyPlayback?.revision, snapshot?.journeyPlayback?.state])
  ...
  return <main className="projection-shell">
    ...
    {snapshot && <section className="projection-canvas">
      ...
      {journey ? <Suspense fallback={<div className="loading-state" role="status">Trazando el recorrido…</div>}><SessionJourneyView journey={journey} /></Suspense> : snapshot.isArchived ? <section className="projection-closing"><h1>Sesión archivada</h1><p>La participación está cerrada. El facilitador conserva el historial de la sesión.</p></section> : closed ? <section className="projection-closing"><SessionConfetti /><p className="section-label">Gracias por participar</p><h1>Sesión finalizada</h1><p>La valoración está cerrada. Gracias por compartir tus ideas.</p><p className="projection-feedback-count">{snapshot.satisfactionSurvey.responseCount} valoraciones recibidas</p></section>
      ...
      : snapshot.questionKind === 'Presentation' ? <AnimatePresence mode="wait"><PresentationSlide key={snapshot.activeQuestionId} title={snapshot.questionTitle} body={snapshot.questionPrompt} settings={snapshot.questionSettings} /></AnimatePresence>
      : <section className="projection-round"><div className="projection-question-row"><div><p className="section-label">{snapshot.sectionTitle} · Pregunta {snapshot.questionIndex}/{snapshot.questionCount}</p><h1>{snapshot.questionTitle}</h1></div>{snapshot.roundOpen && <div className="projection-clock"><span>Tiempo restante</span><strong>{formatTime(getTimeRemaining(snapshot.timerSeconds, snapshot.roundOpen, snapshot.roundOpenedAtUtc, tick))}</strong></div>}</div>
        {snapshot.questionKind === 'Voting' && snapshot.responses.length > 0 ? <VotingResults options={snapshot.options} responses={snapshot.responses} projected /> : snapshot.consolidation?.status === 'Published' && snapshot.consolidation.showConsolidated ? <ProjectedConsolidation groups={snapshot.consolidation.groups} /> : snapshot.responses.length > 0 ? <ProjectedResponses key={snapshot.activeQuestionId} responses={snapshot.responses} /> : <div className="projection-prompt"><p>{snapshot.questionPrompt}</p><div className="projection-guidance"><span className="status-dot" /><p role="status">{snapshot.roundOpen ? 'Es tu turno. Responde desde tu dispositivo.' : snapshot.phase === 'Results' ? 'No hay respuestas públicas para esta pregunta.' : snapshot.roundOpenedAtUtc ? 'Ronda cerrada. Sigue las indicaciones del facilitador.' : 'Hacemos una pausa. Sigue las indicaciones del facilitador.'}</p></div></div>}
