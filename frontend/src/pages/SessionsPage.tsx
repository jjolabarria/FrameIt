...
function SessionCatalog({ routeClientId, routeProjectId }: { routeClientId?: string; routeProjectId?: string }) {
  ...
  const listParams = new URLSearchParams({ q: query, status: params.get('status') ?? '', sort: params.get('sort') ?? 'recent', page: params.get('page') ?? '1', pageSize: params.get('pageSize') ?? '25' })
  ...
  return <WorkspaceLayout title={routeProjectId ? project.data?.name ?? 'Detalle del proyecto' : 'Sesiones'} eyebrow={routeProjectId ? 'Proyecto' : 'Catálogo'} description={routeProjectId ? `${project.data?.clientName ?? ''} · ${project.data?.code ?? ''}` : 'Consulta el historial de todos tus clientes o acota el trabajo por proyecto.'} search={{ value: query, placeholder: 'Buscar sesión, cliente, proyecto o código', onChange: value => update({ q: value }) }} actions={<div className="action-row">{project.data && !invalidContext && <button className="secondary-button" onClick={() => { setEditing(true); update({ crear: '' }, false) }}><Pencil size={16} /> Editar proyecto</button>}<button className="primary-button" disabled={Boolean(routeProjectId && (!project.data || invalidContext))} onClick={() => { setEditing(false); update({ crear: '1' }, false) }}><Plus size={17} /> Crear sesión</button></div>}>
    ...
    {!invalidContext && !project.error && <>
      ...
      {creating && (!routeProjectId || project.data) && <CreateSessionForm clientId={clientId} projectId={projectId} templateId={params.get('templateId') ?? ''} onCancel={() => update({ crear: '' }, false)} onSaved={id => { const back = new URLSearchParams(params); back.delete('crear'); navigate(`/sesion/${id}?returnTo=${encodeURIComponent(location.pathname + (back.size ? `?${back}` : ''))}`) }} />}
      <section className="panel directory-panel"><div className="directory-filters">{!routeProjectId && <><SearchPicker label="Cliente" kind="clients" value={clientId} onChange={id => update({ clientId: id, projectId: '' })} /><SearchPicker label="Proyecto" kind="projects" value={projectId} clientId={clientId} disabled={!clientId} onChange={id => update({ projectId: id })} /></>}<label>Estado<select value={params.get('status') ?? ''} onChange={event => update({ status: event.target.value })}><option value="">Todos los estados</option><option value="Draft">Preparadas</option><option value="Live">En curso</option><option value="Closed">Finalizadas</option></select></label><label>Ordenar por<select value={params.get('sort') ?? 'recent'} onChange={event => update({ sort: event.target.value })}><option value="recent">Actividad más reciente</option><option value="oldest">Actividad más antigua</option><option value="title">Nombre de sesión</option></select></label><button className="ghost-button" onClick={clearFilters}>Limpiar filtros</button></div>
        ...
        {sessions.loading ? <p className="loading-state" role="status">Buscando sesiones…</p> : !sessions.error && sessions.data && <>{sessions.data.items.length ? <SessionDirectory items={sessions.data.items} /> : <EmptyState title="No hay sesiones en esta vista" action={<button className="secondary-button" onClick={clearFilters}>Limpiar filtros</button>}>Revisa los filtros o crea una sesión para este proyecto.</EmptyState>}<Pagination result={sessions.data} onPage={page => update({ page: String(page) }, false)} onSize={size => update({ pageSize: String(size) })} /></>}
      </section>
    </>}
  </WorkspaceLayout>
}
