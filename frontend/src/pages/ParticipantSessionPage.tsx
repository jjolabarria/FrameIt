    ...
    {snapshot && !closed && <>
      {!validIdentity ? <section className="join-surface"><p className="section-label">Te damos la bienvenida</p><h1>{snapshot.title}</h1><p className="join-subtitle">{snapshot.templateTitle}</p>
        ...
        <section className="participant-task">
          ...
          {survey ? <Survey key={identity!.id} snapshot={snapshot} participantId={identity!.id} connected={connected} onSnapshot={setSnapshot} /> : <AnswerForm key={`${snapshot.id}:${identity!.id}:${snapshot.activeQuestionId}`} snapshot={snapshot} identity={identity!} connected={connected} onSnapshot={setSnapshot} />}
