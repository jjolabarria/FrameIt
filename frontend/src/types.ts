...
export type JourneyPlayback = { state: 'Stopped' | 'Playing' | 'Paused'; positionMs: number; startedAtUtc?: string | null; revision: number; durationMs: number; stopCount?: number }
export type JourneyBranch = { label: string; votes: number; isWinner: boolean }
export type JourneyRegion = { id: string; title: string; order: number; capitalCount: number; responseCount: number }
export type JourneyStop = { capitalId: string; sequence: number }
export type JourneyMilestone = { id: string; regionId: string; regionOrder: number; alias: string; kind: 'presentation' | 'question' | 'consolidation' | 'vote'; sectionTitle: string; title: string; metric: number; uniqueContributors: number; topics: string[]; branches: JourneyBranch[]; details: { label: string; text: string }[] }
export type SessionJourney = { sessionId: string; title: string; isInferred: boolean; participantCount: number; responseCount: number; regions: JourneyRegion[]; milestones: JourneyMilestone[]; route: JourneyStop[]; outcomes: OutcomeItem[]; playback: JourneyPlayback }
